...
import { Calendar, Search, Play, RotateCcw, ArrowLeft } from 'lucide-react';
...
export default function SimulatorPage() {
  ...
  const canRunSimulation = selectedETF && startDate && endDate && 
    (investmentType === 'lump' ? parseInt(removeComma(principal)) > 0 : parseInt(removeComma(monthlyAmount)) > 0);
  ...
  return (
    <div className={styles.page}>
      {/* ETF Selection */}
      <Card padding="md" className={styles.etfCard}>
        {!selectedETF && (
          <CardHeader 
            title="ETF 검색"
            subtitle="백테스트할 ETF를 검색하세요"
          />
        )}
      ...
      {/* Input Section */}
      <Card padding="md">
        <Tabs defaultTab="lump" onChange={(tab) => setInvestmentType(tab)}>
          <TabList variant="default">
            <Tab value="lump">거치식 투자</Tab>
            <Tab value="dca">적립식 투자</Tab>
          </TabList>
